...
namespace Battleships
{
    public class HighScoreController
    {
        ...
        private void SaveScores()
        {
            ...
            foreach (Score s in _Scores) output.WriteLine(s.Name + s.Value);
            output.Close();
        }
        ...
        public void DrawHighScores()
        {
            ...
            for (i = 0; i < _Scores.Count - 1; i++)
            {
                ...
                if (i < 9)
                    SwinGame.DrawText(" " + (i + 1) + ":   " + s.Name + "   " + s.Value, Color.White, GameFont("Courier"),
                        SCORES_LEFT, SCORES_TOP + i * SCORE_GAP);
                else
                    SwinGame.DrawText(i + 1 + ":   " + s.Name + "   " + s.Value, Color.White, GameFont("Courier"),
                        SCORES_LEFT, SCORES_TOP + i * SCORE_GAP);
            }
        ...
        public void HandleHighScoreInput()
        {
            if (SwinGame.MouseClicked(MouseButton.LeftButton) || SwinGame.KeyTyped(KeyCode.VK_ESCAPE) ||
                SwinGame.KeyTyped(KeyCode.VK_RETURN)) EndCurrentState();
        }
        ...
        public void ReadHighScore(int value)
        {
            ...
            if (value > _Scores.Item(_Scores.Count - 1).Value)
            {
                ...
                var x = SCORES_LEFT + SwinGame.TextWidth(GameFont("Courier"), "Name: ");
